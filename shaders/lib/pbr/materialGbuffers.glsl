...
void getMaterials(out float smoothness, out float metalness, out float f0, inout float emission,
                  in float subsurface, out float porosity, out float ao, out vec3 newNormal,
    ...
    subsurface += max(specularMap.a > 0.0 ? 1.0 - specularMap.a : 0.0, 0.0);
    ...
    subsurface += max(specularMap.b > 0.251 ? clamp(specularMap.b * 1.335 - 0.355, 0.0, 1.0) : 0.0, 0.0);
